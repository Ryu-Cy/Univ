...
void Life::init(float x, float y)
{
	x_pos = x;
	y_pos = y;
...
// 생명 아이템 클래스
class LifeBonus : public entity
{
public:
	void init(float x, float y);
	void move();
};
void LifeBonus::init(float x, float y)
{
	x_pos = x;
	y_pos = y;
}
...
void initD3D(HWND hWnd)
{
	...
	D3DXCreateFont(d3ddev,               //그래픽 카드 인터페이스
								50,                              // 글자 세로 크기
								25,                               // 글자 가로 크기
								1000,                            // 굵기
								1,                                
...
void init_game(void)
{
	...
	// 생명 초기화
	life[0].init((float)300, 750);
	life[1].init((float)380, 750);
	life[2].init((float)460, 750);
	...
	// 생명 아이템 초기화
	lifeBonus.init((float)(rand() % 500), rand() % 200 - 300);
}
...
void do_game_logic(void)
{
	...
	for (int i = 0; i < ENEMY_NUM * 2; i++)
	{
		...
		{
			enemy[i].init((float)(rand() % 500), rand() % 200 - 300);
			life[entity::myLife - 1].hide();
			entity::myLife--;
		}
	}
	if (hero.check_collision(lifeBonus.x_pos, lifeBonus.y_pos) == true)
	{
		lifeBonus.init((float)(rand() % 500), rand() % 200 - 300);
		if (entity::myLife < 3)
		{
			entity::myLife++;
			life[entity::myLife - 1].active();
		}		
	}

	//적들 처리 
	if (entity::myScore < 10)
	{
...
void render_frame(void)
{
	...
	char point[10];
	sprintf(point, "%d", entity::myScore);

	strcat(score, point);

	RECT rc = { 50, 750, -1, -1 };

	m_pFont->DrawText(NULL, score, -1, &rc, DT_LEFT | DT_NOCLIP, 0xffffff00);
	...
	*/
	...
	for (int i = 0; i < LIFE_NUM; i++)
	{
		if (life[i].show())
		{
			D3DXVECTOR3 position3(life[i].x_pos, life[i].y_pos, 0.0f);    // position at 50, 50 with no depth
	...
	D3DXVECTOR3 position4(lifeBonus.x_pos, lifeBonus.y_pos, 0.0f);    // position at 50, 50 with no depth
	d3dspt->Draw(sprite_heart, &part4, &center4, &position4, D3DCOLOR_ARGB(255, 255, 255, 255));
